...
if ($("#sales-chart").length) {
  ...
  new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        label: 'Pest Levels',
        data: [1, 2, 2, 3, 4], // Data posisi pada Y-axis (sesuai kualitatif)
        ...
        y: {
          ...
          ticks: {
            ...
            callback: function(value) {
              ...
              return labels[value - 1] || ''; // Match the dataset data
            },
            color: "#6C7383",
          },
        },
          ...
          if (legendContainer) {
            ...
            chart.data.datasets.forEach((dataset, i) => {
              ...
              legendItem.innerHTML = `
                <span style="background-color:${dataset.backgroundColor};width:15px;height:15px;display:inline-block;margin-right:10px;"></span>
                ${dataset.label}
              `;
...
if ($("#pest-distribution-chart").length) {
  const ctx = document.getElementById('pest-distribution-chart');
  new Chart(ctx, {
    type: 'doughnut',
    data: {
      labels: ["Hama Wereng", "Ulat Daun", "Tikus Sawah"], // Labels untuk legenda
      datasets: [
        {
          data: [36, 38, 25], // Data persentase
          backgroundColor: ["#006D77", "#83C5BE", "#72A3FF"], // Warna grafik
          borderWidth: 2,
        },
      ],
    },
    options: {
      ...
      plugins: {
        legend: {
          display: false, // Nonaktifkan legenda bawaan
        },
        tooltip: {
          callbacks: {
            label: function (tooltipItem) {
              return `${tooltipItem.label}: ${tooltipItem.raw}%`; // Tambahkan persen ke tooltip
            },
          },
        },
      },
      cutout: '70%', // Membuat donat dengan lubang lebih besar
    },
    plugins: [
      {
        id: 'custom-legend',
        afterUpdate(chart) {
          const legendContainer = document.getElementById("pest-legend");
          legendContainer.innerHTML = ""; // Kosongkan legenda sebelum menambahkan
          chart.data.labels.forEach((label, index) => {
            ...
            legendItem.innerHTML = `
              <span style="
                ...
                width: 12px;
                height: 12px;
                background-color: ${chart.data.datasets[0].backgroundColor[index]};
                margin-right: 8px;
                border-radius: 50%;
              "></span>
              <span>${label}: ${chart.data.datasets[0].data[index]}%</span>
            `;
            legendContainer.appendChild(legendItem);
          });
        },
